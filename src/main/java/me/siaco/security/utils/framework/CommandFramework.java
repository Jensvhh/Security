package me.siaco.security.utils.framework;
	...
	public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (command.consoleOnly() && !(sender instanceof ConsoleCommandSender)){
					sender.sendMessage(ChatColor.RED + "This command may only be executed from the console.");
					return true;
				}
				if (command.isOperatorOnly() && !sender.isOp() && sender instanceof Player){
